...
| `im:resource:upload` | 获取与上传图片或文件资源 | 上传 Copilot 生成或修改后的文件并发回飞书 |

`im:message` 是飞书“获取消息中的资源文件”接口要求的权限之一；文曲星虽然具备该接口
权限，程序仍会在本地强制拒绝群聊和非本人消息。无需申请通讯录、云文档或云盘权限。若组织
启用了审批，提交权限申请并等待管理员批准。

若搜不到权限，先切换到 **应用身份权限**，把搜索条件改为 **权限标识**，再搜索
`im:resource:upload`；也可搜索中文名称“获取与上传图片或文件资源”。部分租户只展示
范围更大的 `im:resource`，开通其中任一项即可。图片或文件上传报 Access denied 时，
重点检查这项权限及应用版本是否已经发布。
